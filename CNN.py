...
class CNN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        log_pobs = F.log_softmax(outputs, dim=1)
        return log_pobs
...
def collate_fn(examples):
    ...
    inputs = pad_sequence(inputs, batch_first=True)  # 对batch内的样本进行padding，使其具有相同长度
...
model = CNN(len(vocab), embedding_dim, filter_size, filter_num, num_class)
model.to(device)
